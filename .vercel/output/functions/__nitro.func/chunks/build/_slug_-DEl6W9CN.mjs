...
import { ref, withAsyncContext, computed, resolveComponent, mergeProps, withCtx, createTextVNode, unref, useSSRContext } from 'vue';
  ...
  async setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isLoading.value) {
        ...
        _push(`<div class="bg-white rounded-lg shadow-lg p-6"><div class="mb-8"><h1 class="text-4xl font-bold text-gray-900 mb-4">${ssrInterpolate(review.value.title)}</h1><div class="flex items-center justify-between mb-6"><div class="flex items-center"><img${ssrRenderAttr("src", review.value.author.avatar_url)}${ssrRenderAttr("alt", review.value.author.full_name)} class="w-10 h-10 rounded-full mr-3"><div><div class="font-medium text-gray-900">${ssrInterpolate(review.value.author.full_name)}</div><div class="text-sm text-gray-500">${ssrInterpolate(formatDate(review.value.created_at))} `);
        ...
        _push(`</div><div class="prose prose-lg max-w-none mb-12"><div>${(_a = unref(renderedContent)) != null ? _a : ""}</div></div><div class="border-t pt-8"><h2 class="text-2xl font-bold text-gray-900 mb-6">Comments</h2>`);
        if (unref(user)) {
          ...
          _push(`<div class="mb-8 text-center">`);
          _push(ssrRenderComponent(_component_NuxtLink, {
            to: "/auth/login",
            class: "btn-primary"
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(` Sign in to comment `);
              } else {
                return [
                  createTextVNode(" Sign in to comment ")
                ];
              }
            }),
            _: 1
        ...
        ssrRenderList(comments.value, (comment) => {
          _push(`<div class="bg-gray-50 rounded-lg p-6"><div class="flex items-center mb-4"><img${ssrRenderAttr("src", comment.author.avatar_url)}${ssrRenderAttr("alt", comment.author.full_name)} class="w-8 h-8 rounded-full mr-3"><div><div class="font-medium text-gray-900">${ssrInterpolate(comment.author.full_name)}</div><div class="text-sm text-gray-500">${ssrInterpolate(formatDate(comment.created_at))}</div></div></div><p class="text-gray-700">${ssrInterpolate(comment.content)}</p></div>`);
        });
        _push(`<!--]--></div></div></div>`);
      }
      _push(`</div>`);
...
//# sourceMappingURL=_slug_-DEl6W9CN.mjs.map
